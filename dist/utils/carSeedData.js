...
exports.generateMockCars = void 0;
...
const generateDubaiLicensePlate = () => {
...
const generateMockCars = (count) => {
    var _a;
    const cars = [];
    for (let i = 0; i < count; i++) {
        const brandData = luxuryBrands[Math.floor(Math.random() * luxuryBrands.length)];
        const modelData = brandData.models[Math.floor(Math.random() * brandData.models.length)];
        const variant = modelData.variants[Math.floor(Math.random() * modelData.variants.length)];
        const location = locations[Math.floor(Math.random() * locations.length)];
        const currentYear = new Date().getFullYear();
        const year = currentYear - Math.floor(Math.random() * 3);
        const power = Math.floor(modelData.powerRange[0] +
        ...
        const car = {
            ...
            type: "luxury",
            color: {
                exterior: colors.exterior[Math.floor(Math.random() * colors.exterior.length)],
                interior: colors.interior[Math.floor(Math.random() * colors.interior.length)],
            },
            vin: generateVIN(),
            ...
            specifications: {
                engine: {
                    ...
                    cylinders: parseInt(((_a = modelData.engineTypes[0].match(/V(\d+)|W(\d+)/)) === null || _a === void 0 ? void 0 : _a[1]) || "8"),
                },
                performance: {
            ...
            availability: {
                ...
                availableFrom: new Date(Date.now() + Math.random() * 604800000).toISOString(),
                availableTo: Math.random() > 0.5
...
exports.generateMockCars = generateMockCars;
